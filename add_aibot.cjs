...
// 処理対象のHTMLファイル（rulesは既にAIボットがあるので除外）
const htmlFiles = [
    'src/index.html',
    'src/matrix.html',
    'src/plans.html',
...
htmlFiles.forEach(filePath => {
    try {
        ...
        // すでにaibotComponentが含まれているかチェック
        if (content.includes('aibotComponent')) {
            console.log(`Skipping ${filePath} - AIbot already exists`);
            return;
        }
        
        // </body>タグの前にAIbotコンポーネントを挿入
        content = content.replace('</body>', aibotHTML + '\n</body>');
        
        fs.writeFileSync(filePath, content, 'utf8');
        console.log(`Added AIbot to ${filePath}`);
    } catch (error) {
        console.error(`Error processing ${filePath}:`, error.message);
    }
});

console.log('AIbot component added to all pages!');